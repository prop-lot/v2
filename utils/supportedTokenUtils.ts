...
import {
  ...
  DELEGATED_VOTES_BY_OWNER_SUB_AT_BLOCK,
  GET_CURRENT_AUCTION,
...
import {
  nounsGraphqlClient,
  lilNounsGraphqlClient,
} from "@/graphql/clients/nouns-graphql-client";
...
export const SupportedTokenGetterMap = {
  [SUPPORTED_SUBDOMAINS.NOUNS]: {
    ...
    getUserTokenCount: async (address: string) => {
      try {
        const data: any = await nounsGraphqlClient
          .query(DELEGATED_VOTES_BY_OWNER_SUB, { id: address.toLowerCase() })
          .toPromise();
    ...
    getUserVoteWeightAtBlock: async (address: string, blockNumber: number) => {
      try {
        const data: any = await nounsGraphqlClient
          .query(DELEGATED_VOTES_BY_OWNER_SUB_AT_BLOCK, {
    ...
    getPreviousAuctions: async (nounIds: number[]) => {
      try {
        const data: any = await nounsGraphqlClient
          .query(GET_PREVIOUS_AUCTIONS, { nounIds })
          .toPromise();
    ...
    getRecentProposals: async () => {
      try {
        ...
        const data: any = await nounsGraphqlClient
          .query(GET_RECENT_PROPOSALS, { recentTimestamp })
          .toPromise();
    ...
    getCandidateData: async (candidateId: string) => {
      try {
        const data: any = await nounsGraphqlClient
          .query(GET_CANDIDATE_DATA, { candidateId })
          .toPromise();
    ...
    getCandidateVotes: async (candidateId: string) => {
      try {
        const data: any = await nounsGraphqlClient
          .query(GET_CANDIDATE_VOTES, { candidateId })
          .toPromise();
    ...
    getAllCandidateData: async (candidateSlugs: string[]) => {
      try {
        const data: any = await nounsGraphqlClient
          .query(GET_ALL_CANDIDATE_DATA, { candidateSlugs })
          .toPromise();

        return data?.data?.proposalCandidates;
      } catch (e) {
        throw new Error("Failed to fetch all candidate data from subgraph");
      }
    },
  },
};
